...
over all records. Instead, we consider the intuitive notion of proportion of the
absolute deviation from the global intercept term as a measure of importance.
Let us consider a particular example to motivate this idea.
...
\begin{itemize}
    ...
    \item  Calculation of the overall importance measures for all features is a
        $\bm{O}(O(p^2 + k))$ operation if we precompute the $Z$ terms.
\end{itemize}
